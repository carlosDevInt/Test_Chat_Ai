 ...
 * the tool set.
 */
type ToolsContextSettings<TOOLS extends ToolSet> = IsEmptyObject<InferToolSetContext<TOOLS>> extends true ? {
    toolsContext?: never;
...
declare function callCompletionApi({ api, prompt, credentials, headers, body, streamProtocol, setCompletion, setLoading, setError, setAbortController, getAbortController, onFinish, onError, fetch, }: {
    api: string;
    prompt: string;
...
type EvaluationModel = string | Experimental_EvaluationModelV4;
...
declare function evaluate<const QUESTIONS extends Record<string, EvaluationQuestion>>({ model: modelArg, state, questions, maxRetries, abortSignal, headers, providerOptions, }: {
 ...
 */
declare function customProvider<LANGUAGE_MODELS extends Record<string, LanguageModel>, EMBEDDING_MODELS extends Record<string, EmbeddingModel>, IMAGE_MODELS extends Record<string, ImageModel>, TRANSCRIPTION_MODELS extends Record<string, TranscriptionModel>, SPEECH_MODELS extends Record<string, SpeechModel>, RERANKING_MODELS extends Record<string, RerankingModel>, VIDEO_MODELS extends Record<string, VideoModel>, FILES extends FilesV4 | undefined = undefined, SKILLS extends SkillsV4 | undefined = undefined, FALLBACK extends ProviderV2 | ProviderV3 | ProviderV4 | undefined = undefined, EVALUATION_MODELS extends Record<string, EvaluationModel> = Record<string, EvaluationModel>>({ languageModels, embeddingModels, imageModels, transcriptionModels, speechModels, rerankingModels, videoModels, evaluationModels, files, skills, fallbackProvider: fallbackProviderArg, }: {
    ...
    evaluationModel(modelId: ExtractModelId<EVALUATION_MODELS>): Experimental_EvaluationModelV4;
...
declare class NoSuchProviderError extends NoSuchModelError {
    ...
    constructor({ modelId, modelType, providerId, availableProviders, message, }: {
        modelId: string;
        modelType: 'languageModel' | 'embeddingModel' | 'imageModel' | 'transcriptionModel' | 'speechModel' | 'rerankingModel' | 'videoModel' | 'evaluationModel';
        providerId: string;
...
type EvaluationProviderRegistry<PROVIDERS extends Record<string, ProviderV4 | ProviderV3> = Record<string, ProviderV4 | ProviderV3>, SEPARATOR extends string = ':'> = ProviderRegistryProvider<PROVIDERS, SEPARATOR> & {
    evaluationModel<KEY extends keyof PROVIDERS>(id: KEY extends string ? `${KEY & string}${SEPARATOR}${ProviderEvaluationModelIdentifier<PROVIDERS[KEY]>}` : never): Experimental_EvaluationModelV4;
    evaluationModel<KEY extends keyof PROVIDERS>(id: KEY extends string ? `${KEY & string}${SEPARATOR}${string}` : never): Experimental_EvaluationModelV4;
};
 ...
 */
declare function toolSearch(): Tool<ToolSearchInput, ToolSearchOutput> & {
    type: 'function';
};
